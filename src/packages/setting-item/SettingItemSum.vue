<template>
  <div class="flex items-center justify-end">
    <img class="mr-1 h-6 w-6" v-if="isHasSumSetting" :src="ImageSuccess" />
    <a-button class="w-20" type="primary" @click="openModal">去设置</a-button>
    <a-modal title="设置求和函数" :visible="isVisible" :width="640" :closable="false" :keyboard="false" @ok="onOk" @cancel="onClose">
      ...
      <template v-else>
        <div v-for="formItem in sumOptions" :key="formItem.formItemId" class="px-6 py-4">
          <div class="flex">
            <div><a-checkbox v-model:checked="formItem.checked" /></div>
            <div class="ml-2">
              <div class="mb-2">{{ formItem.formLabel }}</div>
              <div class="mb-2 flex content-center" v-for="optionItem in formItem.options" :key="optionItem.optionId">
                <span class="mr-2 pt-1">{{ optionItem.label }}</span>
                <div class="inline-block">
                  <a-input v-model:value="optionItem.score" type="number" style="width: 50%" />
                </div>
              </div>
            </div>
          </div>
...
const getOptions = <T extends { optionId: string }>(
  options: T[] | undefined,
  scoreMap: ScoreMapItem
): (T & { score: string | number })[] => {
...
const getSumScoreOptions = () =>
  sumOptions.value.flatMap((formItem) => formItem.options.map((optionItem) => ({ optionId: optionItem.optionId, score: optionItem.score })))
