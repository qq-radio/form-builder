<template>
  <Draggable :group="formItemId" v-model="getSelectOptions" item-key="optionId" ghost-class="drag-ghost"
             class="min-h-full" animation="340" @sort="onOptionChange">
    <template #item="{ element, index }">
      <div class="draggable-area" @mouseenter="onMouseHover(element, index, true)"
           @mouseleave="onMouseHover(element, index, false)">
        <img class="draggable-area__move-icon" v-show="element.isShowMove" :src="ImageOptionMove" />
        <div class="draggable-area__content" v-if="selectConfig">
          <img class="icon" :src="selectConfig.selectIcon" />
          <div class="w-full mx-1">
            <a-textarea :placeholder="selectConfig.placeholder" v-model:value="element.label" @change="onOptionChange"
                        auto-size />
          </div>
          <plus-circle-outlined class="cursor-pointer svg-icon" @click="addOption(index)" />
          <minus-circle-outlined class="ml-1 cursor-pointer svg-icon" v-show="getSelectOptions.length > 1"
...
import { createSelectOptionItemJson } from '../../useFormBuilderJson';
